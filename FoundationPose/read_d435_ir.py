...
## show rgb depth ir
def record_video():
    output_data_dir = r'/home/jiang/Datasets/Yimu/foundation_grasp/adaptor/'
    ...
    if not os.path.exists(os.path.join(output_data_dir, 'ir_left')):
        os.makedirs(os.path.join(output_data_dir, 'ir_left'))
    if not os.path.exists(os.path.join(output_data_dir, 'ir_right')):
        os.makedirs(os.path.join(output_data_dir, 'ir_right'))
    ...
    try:
        ...
        while True:
            ...
            cv2.imwrite(os.path.join(output_data_dir, f'rgb/{frame_id:06d}.png'), ir_left_image)
            cv2.imwrite(os.path.join(output_data_dir, f'ir_left/{frame_id:06d}.png'), ir_left_image)
            cv2.imwrite(os.path.join(output_data_dir, f'ir_right/{frame_id:06d}.png'), ir_right_image)

            cv2.namedWindow('RealSense', cv2.WINDOW_AUTOSIZE)
            cv2.imshow('rgb', color_image)
            cv2.imshow('depth', depth_image*100)
            cv2.imshow('ir_left', ir_left_image)
            cv2.imshow('ir_right', ir_right_image)
